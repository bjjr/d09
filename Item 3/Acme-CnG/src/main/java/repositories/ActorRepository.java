...
public interface ActorRepository extends JpaRepository<Actor, Integer> {
	...
	@Query("select m.sender from MessageEntity m group by m.sender order by count(m) desc")
	List<Actor> findActorWithMoreSentMessages();

	@Query("select m.recipient from MessageEntity m group by m.recipient order by count(m) desc")
